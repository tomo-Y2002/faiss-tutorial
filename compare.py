...
def main():
  ...
  models = [flatL2, ivf]
  time_train_list = [[] for i in range(len(models))]
  time_search_list = [[] for i in range(len(models))]
    ...
    for nq in nq_list:
      for d in d_list:
        ...
        for idx, model in enumerate(models):
          ...
          time_train_list[idx].append(time_train)
          _, _, time_search = model.search(xq)
          time_search_list[idx].append(time_search)
